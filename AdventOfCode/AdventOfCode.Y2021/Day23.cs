...
{
	private List<string> Map { get; } = [.. input.Split("\n")];

	public string Part1()
	{
		var map = Parse([.. Map]);
		var goal = map with { Rooms = new string([.. map.Rooms.Order()]) };

		return SolveMinimumEnergy(map, goal).ToString();
	}
	...
	private static int SolveMinimumEnergy(State start, State goal)
	{
		...
		while (queue.Count > 0)
		{
			...
			if (!visited.Contains(current))
			{
				...
				foreach (var (next, nextCost) in current.GetMoves())
				{
	...
	{
		public static int[] RoomPositions = [2, 4, 6, 8];

		public static char[] AmphipodTypes = ['A', 'B', 'C', 'D'];

		public const int HallwayLength = 11;

		public string GetRoom(int index) => Rooms.Substring(index * RoomSize, RoomSize);

		public IEnumerable<(State state, int cost)> GetMoves()
		{
			for (var i = 0; i < 4; ++i)
			{
				foreach (var pos in GetOpenSpaces(i))
				{
		...
		private bool TryMoveOut(int roomIndex, int targetPosition, out State? updated, out char amphipod, out int steps)
		{
		...
		private bool TryMoveIn(int hallwayPosition, out State? updated, out char amphipod, out int steps)
		{
			updated = default;
			steps = default;
		...
		private IEnumerable<int> GetOpenSpaces(int roomIndex)
		{
